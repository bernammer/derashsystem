...
const UsersListItem = (props) => {
    ...
    const [counterViewCounter] = useCounterViewCounterMutation()
        ...
        <tr key={props.react_unique_identifier_key}>
                ...
                <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div className="form-switch">
                                    <input
                                        type="checkbox"
                                        id={`toggle-status-${props.id}`}
                                        className="sr-only"
                                        checked={props.status ?? false}
                                        onChange={async (e) => {
                                            toggleStatus({id: props.id})
                                            .unwrap()
                                            .then(fulfilled => {
                                            })
                                            .catch(reason => {
                                    ...
                                    />
                                    <label className="bg-slate-400" htmlFor={`toggle-status-${props.id}`}>
